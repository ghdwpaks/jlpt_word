...
target_list = list("不夫付必包法民無末未満約要勇良利陸料量輪")#1
target_list = list("類令冷例歴連老労録以位低失借伝徒働努得特")#2
target_list = list("的底停典加各共協争差最士氏史司臣官府管関")#3
target_list = list("観願信成功固完欠結建健験兵別辺変便仲置単")#4
target_list = list("達帯隊卒孫周祝順初兆億昨季紀希喜旗器機象")#5
target_list = list("照賞案愛札刷折節殺察参産散残倉貯束側続巣")#6
target_list = list("松梅芽菜牧種果材票標械改害街衣囲印英栄塩")#7
target_list = list("胃腸脈康浴養灯堂焼熱毒念敗飯飛費貨課粉副")#8
target_list = list("博告候航望好笑唱泣求救給挙漁軍郡径型景芸")#9
target_list = list("極競鏡省清静席積浅戦選然覚訓議試辞説児治")#10
